...
#    print(model.model.embed_tokens.weight.requires_grad)
  #  if train_head:
   ...
   # if train_embedder:
    ...
    # prepare the dataset
    data_codeforces_cot = load_dataset(cfg.train_dataset,split='train')
...
#load_from_disk("codeforces_cot_filtered")
    tokenized_dataset_train=data_codeforces_cot.select(range(total_examples_per_gpu)).map(helpers.tokenize_codeforce, batched=False,fn_kwargs={
            'context_length':2048,'tokenizer':tokenizer,'include_assistant':False,'padding':True})
